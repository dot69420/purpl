#[cfg(test)]
mod tests {
    use crate::bluetooth::{BtProfile, build_bluetooth_command, run_bluetooth_attacks};
    ...
    fn test_build_bluetooth_command_scan() {
        let (cmd, args) =
            build_bluetooth_command("bluetoothctl", &["scan", "on"], "", false, false);
    ...
    fn test_build_bluetooth_command_target() {
        let (cmd, args) =
            build_bluetooth_command("l2ping", &["-f"], "00:11:22:33:44:55", true, false);
    ...
    fn test_build_bluetooth_command_sudo() {
        let (cmd, args) =
            build_bluetooth_command("l2ping", &["-f"], "00:11:22:33:44:55", true, true);
        assert_eq!(cmd, "sudo");
