...
pub fn view_results(io: &dyn IoHandler) {
    ...
    loop {
        // Level 1: Select Tool (e.g., Nmap, Web, etc.)
        let tools_dir = match fs::read_dir("scans") {
            Ok(d) => d,
            Err(_) => return,
        };
        ...
        crate::clear_screen();
        io.println(&format!("\n{}", "--- Results Viewer ---".cyan().bold()));
        
        for (i, tool) in tool_folders.iter().enumerate() {
            let name = tool.file_name().to_string_lossy().to_string().to_uppercase();
        ...
        let tool_name = selected_tool.file_name().to_string_lossy().to_string().to_uppercase();

        loop {
            // Level 2: Select Target (or Date if flat)
            let mut next_level_items: Vec<_> = fs::read_dir(&tool_path).unwrap()
                .flatten()
                .filter(|e| e.path().is_dir())
                .collect();
            ...
            // Check if immediate children are dates (YYYYMMDD...) or targets
            ...
            if is_dates_only {
                ...
                next_level_items.reverse(); // Newest first
            } else {
                 next_level_items.sort_by_key(|e| e.file_name());
            }

            crate::clear_screen();
            io.println(&format!("\n--- {} > {} ---", "Results".cyan(), tool_name.blue().bold()));
            ...
            io.print(&format!("\n{}", if is_dates_only { "Select Scan: ".green() } else { "Select Target: ".green() }));
            io.flush();
            ...
            if is_dates_only {
                crate::clear_screen();
                display_scan_report(&selected_item.path(), io);
                io.print("\nPress Enter to return...");
                io.flush();
                let _ = io.read_line();
            } else {
                // Level 3: Select Date (if Level 2 was Target)
                loop {
                    let mut dates: Vec<_> = fs::read_dir(selected_item.path()).unwrap()
                        .flatten()
                        .filter(|e| e.path().is_dir())
                        .collect();
                    ...
                    crate::clear_screen();
                    io.println(&format!("\n--- {} > {} > {} ---", "Results".cyan(), tool_name.blue(), selected_item.file_name().to_string_lossy().bold()));
                    ...
                    crate::clear_screen();
                    display_scan_report(&dates[d_idx - 1].path(), io);
                    io.print("\nPress Enter to return...");
                    io.flush();
                    let _ = io.read_line();
                }
