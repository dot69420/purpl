...
mod tests {
    ...
    use crate::sniffer::{SniffProfile, build_sniffer_command, run_sniffer};
    ...
    fn test_run_sniffer_logic_sudo() {
        ...
        assert!(
            calls
                .iter()
                .any(|c| c.args.contains(&"tcpdump".to_string()))
        );
    ...
    fn test_detect_protocol_cases() {
        ...
        assert_eq!(
            detect_protocol("", "POST /login HTTP/1.1"),
            "HTTP (Unencrypted)"
        );
        assert_eq!(
            detect_protocol("", "Some header\nHTTP/1.1 200 OK"),
            "HTTP (Unencrypted)"
        );

        // Credentials
        assert_eq!(
            detect_protocol("", "USER admin"),
            "FTP/Telnet (Credentials)"
        );
        assert_eq!(
            detect_protocol("", "PASS secret"),
            "FTP/Telnet (Credentials)"
        );
