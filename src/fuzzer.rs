...
pub fn configure_fuzzer(
    target: &str,
    global_wordlist: Option<&str>,
    ...
    if !target.contains("FUZZ") {
        io.println(&format!(
            "{}",
            "[!] Target URL must contain the keyword 'FUZZ' where payload should be injected."
                .yellow()
        ));
    ...
    if executor.execute_output("ffuf", &["-V"]).is_err() {
        io.println(&format!(
            "{}",
            "[-] 'ffuf' not found. Please install it (go install github.com/ffuf/ffuf/v2@latest)."
                .red()
        ));
        return None;
    }
    ...
    let wordlist = if let Some(path) = global_wordlist {
        if Path::new(path).exists() {
            ...
            io.println(&format!(
                "{}",
                format!("[!] Custom wordlist '{}' not found.", path).red()
            ));
            return None;
        }
    } else {
